...
import os
import sys
ROOT_DIR = os.path.realpath(__file__).split("demo/demo_det_seg_lane_line_bdd_video_test_speed.py")[0]
...
from dataset.bdd100kLaneandLineSeg import bdd100kLandandLineDataset
import threading
...
class App(QtGui.QMainWindow):
    ...
    def point_degree(self, vector1, vector2):
        """
    ...
    def upsample_back(self, pred, size):
        """ 
    ...
    def lane_post_process_find_mid(self, prob_map, pred_seg_max, img_draw):
        """Self define cluster method, try to find the mid points of each row """
        ###########################
        # get local max & cluster #
        ###########################
        tic = time.time()
        y_value = prob_map.shape[0]-50
        ...
        while(y_value > 50):
            ...
            for point_x in indexes:
                # [distance, direction]
                new_info = [[self.point_distance((cluster[-1][0], cluster[-1][1]), (point_x, y_value)), \
                     self.point_degree(cluster[-1][2], (point_x - cluster[-1][0], y_value - cluster[-1][1]))] for cluster in sampled_points_info]
                ...
                if (min_distance < min_distance_threshold and angle is None) or\
                   (min_distance < min_distance_threshold and angle < 30 and angle is not None):
                    ...
                    new_unit_vector = self.get_unit_vector((point_x, y_value),(sampled_points_info[min_distance_index][-1][0], sampled_points_info[min_distance_index][-1][1]))
    ...
    def main_process(self, frame):
        RGB = np.zeros((self._dataset.input_size[0], self._dataset.input_size[1], 3), dtype = np.uint8)
        with torch.no_grad():
            # if self._count < 2500 or self._count > 7000:
            #     continue
            print("--------------------------")
            start = time.time()
            tic = time.time()
            # resize image
            img = cv2.resize(frame, (self._dataset.input_size[1], self._dataset.input_size[0]), 0, 0, interpolation = cv2.INTER_LINEAR)
            ...
            # net forward
            tic =time.time()
            scores, classification, transformed_anchors, pred_seg_t = self._net(input)
            pred_seg = self._softmax(pred_seg_t)
            toc =time.time()
            ...
            if True:
                ...
                # pred_seg_max_up = self.upsample_back(pred_seg_max, self._dataset.input_size)
                pred_seg_max_up = pred_seg_max
                # RBG for color 
                # print(pred_seg_max.shape)
                # RGB2 = np.zeros((dataset.input_size[0], dataset.input_size[1], 3))
                # flag for overlay area
                # overlay_flag = np.zeros((self._dataset.input_size[0], self._dataset.input_size[1]))
            ...
            self.lane_post_process_find_mid(prob_map, pred_seg_max, img_draw_down)
            end = time.time()
...
if __name__ == "__main__":
    ...
    test_dataset = bdd100kLandandLineDataset(input_size = args.input_size, split = split)
