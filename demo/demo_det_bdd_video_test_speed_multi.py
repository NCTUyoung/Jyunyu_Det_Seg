...
import os
import sys
ROOT_DIR = os.path.realpath(__file__).split("demo/demo_det_bdd_video_test_speed_multi.py")[0]
    ...
    with torch.no_grad():
        while (cap.isOpened()):
            ...
            if ret:
                ...
                ##################
                toc = time.time()
                print("post_process: {}".format(toc- tic))
...
if __name__ == "__main__":
    ...
    # network
    net = net_option(name = args.model, mode = "detection")
    net = net.to(device)
