...
# !IMPORTANT:  No batchnorm at lane subnet
# Date: 2019/03/26
#####################################################################
import os
import time
...
class Jacinto_detseg_lane_v4(nn.Module):
    def __init__(self, num_classes_seg, num_classes_OD, num_classes_lane, pretrained=None, input_shape = [512,1024], mode = "detection"):
        ...
        self.ctx_final = nn.Sequential(nn.Conv2d(64, self.num_classes_seg, 3, 1, 1, 1 ,1),nn.ReLU())
        ...
        # Lane #
        ########
        nn.Sequential(nn.Conv2d(64, self.num_classes_seg, 3, 1, 1, 1 ,1),nn.ReLU())
        self.ctx_conv_up1_1_lane = nn.Sequential(nn.Conv2d(64, 64, 3, 1, 4, 4, 1), nn.ReLU())
        self.ctx_conv_up1_2_lane = nn.Sequential(nn.Conv2d(64, 64, 3, 1, 4, 4, 1), nn.ReLU())
        self.ctx_conv_up1_3_lane = nn.Sequential(nn.Conv2d(64, 64, 3, 1, 1, 1, 1), nn.ReLU())
        self.ctx_conv_up1_4_lane = nn.Sequential(nn.Conv2d(64, 64, 3, 1, 1, 1, 1), nn.ReLU())
        self.ctx_conv_up1_5_lane = nn.Sequential(nn.Conv2d(64, 64, 3, 1, 1, 1, 1), nn.ReLU())
        self.ctx_conv_up2_1_lane = nn.Sequential(nn.Conv2d(64, 64, 3, 1, 4, 4, 1), nn.ReLU())
        self.ctx_conv_up2_2_lane = nn.Sequential(nn.Conv2d(64, 64, 3, 1, 4, 4, 1), nn.ReLU())
        self.ctx_conv_up2_3_lane = nn.Sequential(nn.Conv2d(64, 64, 3, 1, 1, 1, 1), nn.ReLU())
        self.ctx_conv_up2_4_lane = nn.Sequential(nn.Conv2d(64, 64, 3, 1, 1, 1, 1), nn.ReLU())
        self.ctx_conv_up2_5_lane = nn.Sequential(nn.Conv2d(64, 64, 3, 1, 1, 1, 1), nn.ReLU())

        self.ctx_conv_up3_lane = nn.Sequential(nn.Conv2d(64, 64, 3, 1, 1, 1, 1), nn.ReLU())
        self.ctx_conv_up3_class = nn.Sequential(nn.Conv2d(64, self.num_classes_lane, 3, 1, 1, 1, 1), nn.ReLU())
