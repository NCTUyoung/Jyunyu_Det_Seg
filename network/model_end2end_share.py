...
class PyramidFeatures(nn.Module):
    ...
    def forward(self, inputs):
        ...
        print(p3.size(), p4.size(), p5.size(), p5.size())
...
class ResNet(nn.Module):
    ...
    def forward(self, inputs):
        if self.mode == "detection":
            ...
            features_pyramid = self.fpn([c2 ,c3, c4, c5])
            # print(features_pyramid[0].size(), features_pyramid[1].size())
            # print(features_pyramid[2].size(), features_pyramid[3].size())
            # print(features_pyramid[4].size())
            ...
            ##############################
            d2 = self.D2_conv_3x3_1(features_pyramid[0])
            d2 = self.D2_conv_3x3_2(d2)
            d3 = self.D3_conv_3x3_1(features_pyramid[1])
            d3 = self.D3_conv_3x3_2(d3)
            d4 = self.D4_conv_3x3_1(features_pyramid[2])
            d4 = self.D4_conv_3x3_2(d4)
            d5 = self.D5_conv_3x3_1(features_pyramid[3])
            d5 = self.D5_conv_3x3_2(d5)
            # d6 = self.D5_conv_3x3_1(features_pyramid[4])
            # d6 = self.D5_conv_3x3_2(d6)
            # print(d2.size(),d3.size(),d4.size(),d5.size(),d6.size())
            # print("d3.size(), d4.size(), d5.size()",d3.size(), d4.size(), d5.size())
            d3 = self.upsample3(d3)
            d4 = self.upsample2(d4)
            d5 = self.upsample1(d5)
            # d6 = self.upsample0(d6)
            ...
            #############################
            regression = torch.cat([self.regressionModel(feature) for feature in features_pyramid], dim=1)
            classification = torch.cat([self.classificationModel(feature) for feature in features_pyramid], dim=1)
