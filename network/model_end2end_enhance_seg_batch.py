...
class ResNet(nn.Module):
    ...
    def __init__(self, num_classes_seg, num_classes_OD, block, layers, mode = "detection"):
        ...
        self.freeze_bn2()
        # for index, para in enumerate(self.bn1.parameters()):
        ...
        self.conv_final_smooth_bn = nn.BatchNorm2d(128)
        # for index, para in enumerate(self.conv_final_smooth_bn.parameters()):
        #     print(para.requires_grad)
        """ extran convolution for upsample  """
        self.conv_up1 = nn.Conv2d(128,128,kernel_size = 3, stride = 1, padding = 1) 
        self.conv_up1_bn = nn.BatchNorm2d(128)
        self.conv_up2 = nn.Conv2d(128,128,kernel_size = 3, stride = 1, padding = 1) 
        self.conv_up2_bn = nn.BatchNorm2d(128)
    ...
    def freeze_bn(self):
        ...
        for layer in self.modules():
            if isinstance(layer, nn.BatchNorm2d):
                layer.eval()
    def freeze_bn2(self):
        '''Freeze BatchNorm layers. alpha and beta'''
        for layer in self.modules():
            # print(layer.__dict__["_modules"])
            if isinstance(layer, nn.BatchNorm2d):
                for para in layer.parameters():
                    para.requires_grad = False 
    # def freeze_bn_only_backbone(self, fix_layer):
    #     count = 0
    #     for name, module in net.named_children():
    #         if name in fix_layer:
    #             count += 1
    #             for para in module.parameters():
    #                 para.requires_grad = False
    #     print("Fixed resnet backbone: {}/{}".format(count,len(resnet_backbone_parameters)))
