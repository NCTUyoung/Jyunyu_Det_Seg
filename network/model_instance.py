...
class ResNet(nn.Module):
    ...
    def __init__(self, num_classes_seg, num_classes_OD, block, layers, mode = "detection"):
        ...
        self.conv_final = nn.Conv2d(256, num_classes_seg, kernel_size = 1, stride = 1, padding = 0) # num_classes = 19
    ...
    def freeze_bn(self):
        '''Freeze BatchNorm layers Running mean and variance'''
    ...
    def forward(self, inputs):
        if self.mode == "detection":
            ...
            ##############################
            d2 = self.D2_conv_3x3_1(k2)
            d2 = self.D2_conv_3x3_2(d2)
            d3 = self.D3_conv_3x3_1(k3)
            d3 = self.D3_conv_3x3_2(d3)
            d4 = self.D4_conv_3x3_1(k4)
            d4 = self.D4_conv_3x3_2(d4)
            d5 = self.D5_conv_3x3_1(k5)
            d5 = self.D5_conv_3x3_2(d5)
            ...
            pred_seg = self.conv_final(d_smooth)
            # if self.target_available:
            ...
            if self.training:
                # detection
                focal_loss = self.focalLoss(classification, regression, anchors, annotations)
                # segmentation
                pred_seg = self.upsample_back(target, pred_seg)
                seg_loss = self.seg_loss(pred_seg, target)
    ...
    def forward_segmentation(self, inputs):
        ...
        # print("self.conv1 b",self.conv1.bias)
        # print("conv1",x)
        x = self.bn1(x)
        # print("self.bn1",self.bn1.weight)
        # print("self.bn1 b",self.bn1.bias)
        # print("self.bn1 mean",self.bn1.running_mean)
        # print("self.bn1 var",self.bn1.running_var)
        # print("bn1",x)
        # yyy= input("pause")
        # print(self.bn1.weight)
        x = self.relu(x)
        ...
        c5 = self.layer4(c4)
        # print("layer4",c5)
        # print(self.layer4[0].bn1.weight)
        [features_segmentation, features_detection] = self.fpn([c2 ,c3, c4, c5])
        (k2, k3, k4, k5) = (features_segmentation[0], features_segmentation[1], features_segmentation[2], features_segmentation[3])
