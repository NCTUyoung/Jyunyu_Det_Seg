...
class PyramidFeatures(nn.Module):
    ...
    def forward(self, inputs):
        C2, C3, C4, C5 = inputs # C3 size :(1, 512, 64, 128) 1/8x , C4 size :(1, 1024, 32, 64), C5 size :(1, 2048, 16, 32) 
        ####################
        # detection subnet #
        ####################
        p5 = self.P5_conv(C5)
...
class ResNet(nn.Module):
    ...
    def __init__(self, num_classes_seg, num_classes_OD, block, layers, mode = "detection"):
        ...
        self.regressionModel = RegressionModel(256 + 8, num_anchors = 15)
        self.classificationModel = ClassificationModel(256 + 8, num_anchors = 15, num_classes=num_classes_OD)
    ...
    def forward(self, inputs):
        if self.mode == "detection":
            ...
            d_smooth = F.relu(self.conv_final_smooth(d_final)) # 128x256
            ##################################
            # extra conv to upsmaple back
            ##################################
            d_up1 = self.upsample3(d_smooth) # 256x512
            d_up1 = F.relu(self.conv_up1(d_up1)) #
            d_up2 = self.upsample3(d_up1) # 512x1024
            d_up2 = F.relu(self.conv_up2(d_up2)) 
            pred_seg = self.conv_final(d_up2)
            # print(pred_seg.size())
            # print(features_detection[0].size(),features_detection[1].size(),features_detection[2].size())
            # print(features_detection[3].size(),features_detection[4].size())
            # print("---------------")
            ################################################
            # concate segmentaion output to detection input
            ################################################
            pred_seg_down = self.maxpool(pred_seg[:,11:,:,:]) # only obj channel
            pred_seg_down = self.maxpool(pred_seg_down)
            pred_seg_down = self.maxpool(pred_seg_down)
            features_detection[0] = torch.cat([pred_seg_down, features_detection[0]], dim = 1)
            pred_seg_down = self.maxpool(pred_seg_down)
            features_detection[1] = torch.cat([pred_seg_down, features_detection[1]], dim = 1)
            pred_seg_down = self.maxpool(pred_seg_down)
            features_detection[2] = torch.cat([pred_seg_down, features_detection[2]], dim = 1)
            pred_seg_down = self.maxpool(pred_seg_down)
            features_detection[3] = torch.cat([pred_seg_down, features_detection[3]], dim = 1)
            pred_seg_down = self.maxpool(pred_seg_down)
            features_detection[4] = torch.cat([pred_seg_down, features_detection[4]], dim = 1)
            # print(features_detection[0].size(),features_detection[1].size(),features_detection[2].size())
            # print(features_detection[3].size(),features_detection[4].size())
            # print("---------------==")
            #############################
...
def resnet152(num_classes_seg, num_classes_OD, pretrained=None, mode = "detection", **kwargs):
