...
class Anchors(nn.Module):
    def __init__(self, input_shape = None,pyramid_levels=None, strides=None, sizes=None, ratios=None, scales=None, double = None):
        ...
        if pyramid_levels is None:
            if self.double is None:
                ...
                self.pyramid_levels = [3, 3, 4, 4, 5, 5, 6, 6, 7, 7]
        if strides is None:
        ...
        if sizes is None:
            ...
            ############################
            # self define anchor boxes #
            ############################
            """ For 1024x512 """
            # self.sizes = [20.48, 35.84, 51.20, 153.60, 256.0, 358.34]
            """ For 256x512 """
            # self.sizes = [10.24, 17.92, 25.60, 76.30, 128.0, 179.17]
        if ratios is None:
        ...
        if scales is None:
            ...
            self.scales = scales
            # self.scales = np.array([2 ** 0, 2 ** (1.0 / 3.0), 2 ** (2.0 / 3.0), 2 ** (-1.0 / 3.0), 2 ** (-2.0 / 3.0)])
    ...
    def forward(self, image):
        ...
        for idx, p in enumerate(self.pyramid_levels):
            ...
            if self.double:
                if (idx) % 2 == 0:
                    shifted_anchors = shift(image_shapes[idx], self.strides[idx], anchors, 0.5)
                else:
                    shifted_anchors = shift(image_shapes[idx], self.strides[idx], anchors, 1.0)
            else:
                shifted_anchors = shift(image_shapes[idx], self.strides[idx], anchors, 0.5)
