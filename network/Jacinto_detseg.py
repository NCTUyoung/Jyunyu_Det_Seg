...
class Jacinto_detseg(nn.Module):
    ...
    def forward_lane_bdd(self, inputs):
        if self.target_available:
            img_batch, seg_gt = inputs
        else:
            img_batch = inputs
        x = self.conv1a(img_batch) # 1024x 512
        x = self.conv1b(x) 
        ...
        if self.target_available:
            ...
            return loss_seg, pred_seg      
        else:
            return pred_seg
"""
    def forward_lane_line_bdd(self, inputs):
        if self.target_available:
            img_batch, line_gt, lane_gt = inputs
        else:
            img_batch = inputs
        x = self.conv1a(img_batch) # 1024x 512
        x = self.conv1b(x) 
        ...
        x = self.ctx_conv2(x)
        x = self.ctx_conv3(x)
        out_temp = self.ctx_conv4(x)
        ########
        # line #
        ########
        x = self.ctx_conv_up1_lane(out_temp)
        x = self.ctx_conv_up2_lane(x)
        # segmenation output
        x = self.ctx_conv_up3_lane(x)
        pred_line = self.ctx_final_lane(x)
        ########
        # lane #
        ########
        x = self.ctx_conv_up1(out_temp)
        x = self.ctx_conv_up2(x)
        x = self.ctx_conv_up3(x)
        pred_lane = self.ctx_final_(x)
        # print("pred_cls.size()", pred_cls.size())
        # print("pred_seg.size()", pred_seg.size())
        if self.target_available:
            # line segmentation
            pred_line = self.upsample_back(line_gt, pred_line)
            loss_line = self.lane_loss(pred_line, line_gt)
            # lane segmetation
            pred_lane = self.upsample_back(lane_gt, pred_lane)
            loss_lane = self.seg_loss(pred_lane, lane_gt)
            return loss_line, pred_line, loss_lane, pred_lane
        else:
            return pred_seg, pred_lane
"""
...
class Jacinto_detseg_OD_GUIDE_v1(nn.Module):
    ...
    def forward(self, inputs):
        ...
        #    return self.forward_lane_line_bdd(inputs)
            ...
            reverse_response = soft_pred
            # print(reverse_response.size())
            reverse_response.unsqueeze_(-1)
            # print(reverse_response.size())
            reverse_response = reverse_response.expand(reverse_response.size()[0], reverse_response.size()[1], reverse_response.size()[2], 64)
            reverse_response = reverse_response.transpose(1,2).transpose(1,3)
            # print(reverse_response.size())
            # print(reverse_response.size())
            # # print(reverse_response.size(), out5a_combined.size())
            offset_feature = reverse_response * out5a_combined
    ...
    def forward_detection(self, inputs):
        ...
        # guided_feature = guided_feature[0,1,:,:]
        # out5a_combined_v = out5a_combined_v.data.cpu().numpy()
        # guided_feature = guided_feature.data.cpu().numpy()
        # print("out5a_combined", np.max(out5a_combined_v), np.min(out5a_combined_v))
        # print("guided_feature", np.max(guided_feature), np.min(guided_feature))
        # out5a_combined_v *= 255
        # guided_feature *= 255
        # cv2.imshow("seg_out", out5a_combined_v)
        # cv2.waitKey(0)
        # cv2.imshow("seg_out", guided_feature)
        # cv2.waitKey(0)
        ############## 
        # DEBUG SHOW #
        ##############
...
class Jacinto_detseg_OD_GUIDE_v2(nn.Module):
    def __init__(self, num_classes_seg, num_classes_OD, num_classes_lane, pretrained=None, mode = "detection"):
        super(Jacinto_detseg_OD_GUIDE_v2, self).__init__()
        self.num_classes_seg = num_classes_seg
        self.num_classes_OD = num_classes_OD
        self.num_classes_lane = num_classes_lane
        self.pretrained = pretrained
    ...
    def forward_detection(self, inputs):
        ...
        # segmentation guide detection #
        ################################
        # decrease background response, mantain foreground response
        seg_pred_down = self.poolind2d(seg_pred)
        seg_pred_down = self.poolind2d(seg_pred_down)
        seg_pred_down = self.poolind2d(seg_pred_down)
        soft_pred = self.softmax(seg_pred_down)
        soft_pred = soft_pred[:, 0, :, :]
        soft_pred[soft_pred < 0.5] = 0.0
        soft_pred[soft_pred > 0.5] = 1.0
        # seg_pred_max = soft_pred.max(1)[0]
        reverse_response = soft_pred
        reverse_response.unsqueeze_(-1)
        # print(reverse_response.size())
        ...
        # print(reverse_response.size())
        # # print(reverse_response.size(), out5a_combined.size())
        guided_feature = reverse_response * out5a_combined
        # guided_feature = out5a_combined + offset_feature
        # print(guided_feature.size(), out5a_combined.size())
        # print(guided_feature.size())
        # print(guided_feature[0,0,56,56], reverse_response[0,0,56,56], out5a_combined[0,0,56,56])
