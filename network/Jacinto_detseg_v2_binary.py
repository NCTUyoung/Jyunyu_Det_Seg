...
from loss_weighted_binary import weighted_binary_cross_entropy
...
class Jacinto_detseg_v2_binary(nn.Module):
    def __init__(self, num_classes_seg, num_classes_OD, num_classes_lane, pretrained=None, input_shape = [512,1024], mode = "detection"):
        ...
        self.ctx_conv_up3_class = self._make_conv_block(64, self.num_classes_lane-1, 3, 1, 1, 1, 1)
        self.ctx_final_lane = nn.Sequential(nn.Conv2d(64, self.num_classes_lane-1, 3, 1, 1, 1 ,1),
                            nn.ReLU())
        self.sigmoid = nn.Sigmoid()
        self.binary_class_loss = torch.nn.BCELoss()
        self.binary_class_loss_2 = torch.nn.BCELoss()
    ...
    def upsample_back(self, target, pred):
        ...
        (2) pred  : network prediction [b, C, H, W]
        Return 
    ...
    def upsample_back_v2(self, target, pred):
        """This function upsample pred to target's size
        Args:
        (1) target: segmentation target [b, C, H, W]
        (2) pred  : network prediction [b, C, H, W]
        Return 
        (2) pred  : prediction with same size with target
        """
        # print("target.size(), pred.size()", target.size(), pred.size())
        if (target.size()[2], target.size()[3]) != (pred.size()[2], pred.size()[3]):
            pred = F.interpolate(pred, (target.size()[1], target.size()[2]), mode='nearest')
            # print(pred.size())
            return pred
        else :
    ...
    def forward_lane(self, inputs):
        ...
        pred_seg = self.sigmoid(pred_seg)
        ...
        # print("seg_gt", seg_gt.size())
        if self.target_available:
            # segmentation
            pred_seg = self.upsample_back_v2(seg_gt, pred_seg)
            # loss_seg = self.binary_class_loss_2(pred_seg, seg_gt)
            loss_seg = weighted_binary_cross_entropy(pred_seg, seg_gt, [1.0,0.0001])
